...
\begin{itemize}
    ...
    \item Rubin Observatory Science Collaborations will need to monitor progress to plan scientific analysis and provide feedback to the Rubin Observatory Survey Cadence Optimization Committee (SCOC) so it can be fed back into plans for updating Rubin Observory scheduling.
\end{itemize}
...
\begin{itemize}
    ...
    \item Web application frameworks can be used to drive the necessary calls to \texttt{schedview} functions and create a full, interactive dashboard web applications. For example, the \texttt{schedlure_dashboard} tool is \texttt{phalanx} application implemented by combining calls to \texttt{schedview} tools using the \texttt{panel} dashboard library.
\end{itemize}

The separation of the visualization generation from report compilation in \texttt{schedview}'s architecture should make it straightforward to include its visualizations in reports generated by other python tools as well.
...
\begin{itemize}
    ...
    \item A map of the visits completed on the night plotted over the accumulated depth of the survey so far, organized into subplots by band. Airmass limit lines for the start and and of the night indicate the limits of what could be observed on the night. Figure~\ref{fig:accumdepth} shows an example of such a plot, and \texttt{schedview/examples/accumdepth.py} shows a minimal workflow to produces such a plot in a file. This figure serves as an intuitive diagnostic of whether the scheduler observing pointings that will result in uniform coverage.
    \item A map of the visits completed on the night plotted over the time since the last time that pointing in the sky was observed, organized into subplots by band. Airmass limit lines for the start and and of the night indicate the limits of what could be observed on the night. Figure~\ref{fig:agemap} shows an example of such a plot, and \texttt{schedview/examples/agemap.py} shows a minimal workflow to produces such a plot in a file. This figure serves as an intuitive diagnostic of how well the scheduler is maintaining an observing cadence, and in what regions of the sky.
    \item A plot showing the recent depths and dates of sequences of visits on LSST deep drilling fields \todo{reference}. It consists of panes stacked vertically, one for each deep drilling field. Within each pane, vertical bars representing nights of observing are arranged along the horizontal axis. The height of each bar represents the depth (as on effective exposure time \todo{reference}) in each band accumulated on that night. Figure~\ref{fig:ddfcadence} shows an example of such a plot, and \texttt{schedview/examples/ddfcadence.py} shows a minimal workflow to produces such a plot in a file.
...
{
    \centering
    \includegraphics[width=1\linewidth]{accumdepth.png}
    \caption{Accumulated depth map}
    \label{fig:accumdepth}
}

Each subplot represents the progress in the survey in a different
filter at the start of a night, presented in a
\href{https://en.wikipedia.org/wiki/Lambert_azimuthal_equal-area_projection}{Lambert
Azimuthal Equal Area Projection}, centered at the south celestial pole.

Annotations are similar to those of the ``Visit Map'' (Figure~\ref{fig:visitmap}):

\begin{itemize}
\item
  Black outlines show the camera footprint of each exposure taken on
  this night.
\item
  The blue backdrop represents the accumulated effective exposure time,
  teff (as defined above) at the start of the night.
\item
  The orange disk shows the coordinates of the moon.
\item
  The yellow disk shows the coordinates of the sun.
...
\item
  The blue line (oval) shows the plane of the Milky Way.
\item
  Broken red ovals show a zenith distance of 70\degree (airmass=2.9) at
  morning and evening 12° twilight.

  \begin{itemize}   
  \item
    The dashed portions of each red oval show the extent of what is
    observable at all during the night.
  \item
    As time progress through the night, the ZD circle maintains its
    shape, moving counter-clockwise around the pole over the course of
    the night.
  \end{itemize}
\end{itemize}

Over-plotted on the depth map are black outlines of visits taken on the night.
The combination of the broken red ovals (which show the limits of what can be observed on the night) and the blue representation of the depth provide a rough indication of whether the scheduler is selecting visits that ``fill in'' shallow areas in survey depth.
...
{
    \centering
    \includegraphics[width=1\linewidth]{agemap.png}
    \caption{Age map}
    \label{fig:agemap}
}
...
Other features and annotations are the same as those of the depth map (Figure~\ref{fig:accumdepth}).

Over-plotted on the depth map are black outlines of visits taken on the night.
The combination of the broken red ovals (which show the limits of what can be observed on the night) and the blue representation of the depth provide a rough indication of how well the scheduler was selecting visits to maintain an observing cadence.
...
High-level reports such as those that need to be presented to upper management, the scientific collaborations, and funding agencies will be written by humans, rather than generated with automatically executed reports and dashboards. However, these reports will require an assortment of figures, and the \schedview package will host reusable code that generates such figures, particularly code that can be reused and refined over time for successive reports.

Survey progress reports will require many of the same kinds of figures already produced by MAF for assessment of survey strategy simulations. In some cases, figures for progress reports may be generated using MAF utilities directly, and \schedview may not be involved. In others cases, \schedview will provide additional functionality. \schedview currently includes infrastructure for incorporating MAF tools into its own workflow and reporting, allowing the integration of functionality between \schedview and MAF: the \texttt{schedview.compute.maf} submodule uses arbitrary MAF slicers and metrics to perform the ``compute'' stage of the \schedview workflow, and MAF's plotting and plot handling functionality can typically be used directly as the ``plot'' stage. It is also possible to mix-and-match implementations in the MAF and \schedview infrastructure, and indeed this is what is done in the depth and age plots shown in figures~\ref{fig:accumdepth} and~\ref{fig:agemap}: \schedview calls MAF functions to compute the depth or age over the sky, and then uses its own plotting tools to generate the figure itself.

Another example of a MAF plot that will be useful in progress reports are hourglass plots, as show in Figure~\ref{fig:hourglass}. Hourglass plots are particularly useful for progress reporting because they show data that varies on a visit-scale timescale over longer time periods (up to a year for a singly reasonably sized figure) and its astronomical context (time of year and night, time relative to sun and moon rise and set).
